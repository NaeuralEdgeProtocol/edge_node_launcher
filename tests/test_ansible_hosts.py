...
from models import AnsibleHosts as model_ansible_hosts
...
def test_ansible_hosts_manager_preserves_quoted_common_args_and_key_paths(tmp_path, monkeypatch):
    hosts_file = (
        tmp_path
        / ".ansible"
        / "collections"
    ...
    gpu_nodes:
      hosts:
        edge-a:
          ansible_host: 192.0.2.10
          ansible_user: ratio
          ansible_ssh_private_key_file: ~/.ssh/edge key
          ansible_ssh_common_args: '-o ProxyCommand="ssh -W %h:%p bastion" -o StrictHostKeyChecking=no'
""",
        encoding="utf-8",
    )
    ...
    assert manager.get_ssh_command_prefix("edge-a") == [
        ...
        "StrictHostKeyChecking=no",
        "-i",
        str(tmp_path / ".ssh" / "edge key"),
        "ratio@192.0.2.10",
    ]


def test_model_ansible_hosts_manager_exposes_structured_and_display_commands(tmp_path, monkeypatch):
    hosts_file = (
        tmp_path
        / ".ansible"
        / "collections"
...
""",
    ...
    monkeypatch.setattr(model_ansible_hosts.os.path, "expanduser", fake_expanduser)

    manager = model_ansible_hosts.AnsibleHostsManager()
    parts = manager.get_ssh_command_parts("edge-a")
    ...
    assert manager.get_ssh_command("missing") is None
    assert model_ansible_hosts.split_ssh_args(manager.get_ssh_command("edge-a")) == parts
